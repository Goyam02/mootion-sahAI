...
export function StudentLoginPage() {
  ...
  const handleStart = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (data.role !== 'student') {
        setError("This account is registered as a teacher. Please log in via the teacher portal.");
        setIsLoading(false);
        return;
      }
  ...
  return (
    <div className="w-full h-[100dvh] bg-[#f6f4ee] overflow-hidden relative flex items-center justify-center">
      ...
      >
        <h2 className="text-center tracking-normal sm:tracking-[0.1em] md:tracking-[0.15em] text-[#2c2c2c] uppercase text-[10px] min-[320px]:text-[11px] min-[360px]:text-[12px] sm:text-[14px] md:text-base font-black whitespace-nowrap">Student Login</h2>
        ...
        <form onSubmit={handleStart} className="flex flex-col flex-1 pb-4">
          <div className="flex flex-col gap-3 md:gap-4 flex-1 justify-center relative mt-2">
            ...
            <input 
              ...
              disabled={isLoading}
              className="w-full px-6 py-2 md:py-3 text-[13px] sm:text-sm md:text-base bg-transparent border border-[#1800ad] rounded-full text-center text-[#2c2c2c] placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#1800ad]"
            />
            <input 
              type="password" 
